...
export async function signin({ email, password }) {
  ...
  const shaHashed = sha256Hash(password); // hash before sending
...
export async function signinAdmin({ email, password }) {
  if (USE_MOCK) {
    ...
    const match = await bcrypt.compare(password, admin.hashed);
    if (!match) return { ok: false, error: "Invalid credentials" };
    const session = { user: { name: admin.name || email.split("@")[0], email, role: "admin" }, token: "mock-token" };
    saveSession(session);
    return { ok: true, data: session };
  }
  ...
  if (!res.ok)
    return { ok: false, error: (await res.json()).error || "Admin signin failed" };
